...
-- The columns named "country," "worldwide_gross_income," "languages," and "production_company" have null values. 
...
FROM   movie
GROUP  BY year; 
-- In the year 2017 had the highest number of movies released.
...
GROUP  BY month_num
ORDER  BY month_num; 
-- In the month of March had the highest number of movie releases.
...
WHERE  ( country LIKE '%INDIA%'
          OR country LIKE '%USA%' )
       AND year = 2019;
-- Both the USA and India produced over a thousand movies (1059 to be exact) in the year 2019.
...
-- Drama movies have the highest movie count among all genres, with a count of 4285.
...
-- There are 3289 movies in the dataset that belong to only one genre.
...
-- Action genre has the highest average duration of 112.88, followed by the Romance genre with an average duration of 109.53, and the Crime genre with an average duration of 107.05.
...
-- Thriller genre has rank 3 in terms of the number of movies produced, with a count of 1484.
...
-- Top 2 movies have an average rating 10.0 out of 10.0.
...
-- Among all the movies, those with a median rating of 7 have the highest movie count.
...
-- They both have rank 1 and movie count 3 
...
-- In March 2017 in the USA, the top three movie genres based on the number of movies with over 1,000 votes were Drama, Comedy, and Action, with 24, 9, and 8 movies respectively.
...
-- There are 8 movies which begin with "The" in their title and which have an average rating > 8.
...
-- There are 361 movies that were released between April 1st, 2018 and April 1st, 2019 with a median rating of 8.
...
-- There are two approach to solve this query:
...
SELECT country,
       ...
       IF(SUM(CASE
                WHEN country = 'Germany' THEN total_votes
                ELSE 0
              END) > SUM(CASE
                           WHEN country = 'Italy' THEN total_votes
                           ELSE 0
                         END), 'Yes', 'No') AS Answer
FROM   movie AS m
       INNER JOIN ratings AS r
...
-- We found that German movies received the highest number of votes based on both the language and country columns.
...
-- The columns named "height," "date_of_birth," and "known_for_movies" have null values.
...
-- James Mangold, Anthony Russo and Soubin Shahir are the top three directors whose movies have an average rating greater than 8.
...
-- Top 2 actors are Mammootty and Mohanlal whose movies have a median rating greater than or equal to 8.
...
-- Top 3 Genres based on most number of movies
	...
	movie_summary AS (
		...
		GROUP BY year, movie_name, genre, worlwide_gross_income
	)
...
-- Avengers is highest-grossing movie overall(As well as both the years 2018 and 2019).
